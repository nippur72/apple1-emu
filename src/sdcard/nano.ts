   ...
   step() {
      ...
      if(this.state == "idle") {
         ...
         if(this.receive_buffer.length == this.bytesleft) {
            ...
            let file_ok = this.sdcard.writeFile(this.filename, new Uint8Array(this.receive_buffer));
            if(file_ok) {
               this.receive_buffer = [];
               this.send_buffer.push(OK_RESPONSE);
               this.state = "send";
         ...
         if(this.data == 0) {
            ...
            // seeks for file            
            if(match != undefined && this.sdcard.exist(match)) {
               filename = match;
               if(this.sdcard.isDirectory(filename)) {
                  this.send_buffer.push(ERR_RESPONSE);
                  this.send_buffer.push(...stringToArray("?NOT A FILE\0"));
         ...
         if(this.data == 0) {
            ...
            let parent = function(fullpath: string) {               
...
export function nano_byte_sent(data: byte) {
   return nano.byte_sent(data);
}

export function nano_timeout(nano_state: number) {
