...
async function readFile(fileName: string) {
   throw "not impemented";
   const bytes = await idb.get<Uint8Array>(fileName, store);   
   return bytes;
}

async function writeFile(fileName:string, bytes: number[]) {  
   throw "not impemented";
...
async function load(filename: string, p: any) {
   throw "not impemented";
...
async function save(filename: string, p1: any, p2: any) {
   throw "not impemented";
   const ext = filename.substr(-4).toLowerCase();
...
export function loadBytes(bytes: Uint8Array, fileName?: string) {   
   let buffer = bytes;
...
function do_load(buffer: any, num_bytes: number) {   
   apple1.load_prg(buffer, num_bytes);
}

async function load_file(fileName: string, address: number) {   
   throw "not impemented";
   /*
...
async function save_file(filename: string, start: number, end: number) {
   throw "not impemented";
   /*
